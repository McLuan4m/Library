...
return function(Library)
    ...
    function Page:AddSection(config)
        ...
        table.insert(self._sectionWidgets, section)
        return section
    end
    ...
    function Page:SetActive(active)
        ...
        -- Ao ativar, faz as seções entrarem em cascata (stagger). Como as
        -- seções vivem dentro de um UIListLayout (que controla a posição),
        -- animamos via UIScale + transparência, que são independentes do
        -- layout — evitando conflito com o posicionamento automático.
        if active then
            local RenderEngine = Library.Core.RenderEngine
            for i, section in ipairs(self._sectionWidgets) do
                if section.Instance then
                    section:PlayEntrance((i - 1) * 0.05)
                end
            end
        end
    end
